...
PRIVATE:
    template<class __Ty> struct _Node {
        __Ty _Myval;
        vector<_Node<__Ty> *> _Forward;
...
public:
    skiplist() : _Nodecnt(0), _Level(0), _Dummy(new _Node<value_type>(0, max_level)) {}
    skiplist(const initializer_list<value_type> &_Ilist) : _Nodecnt(_Ilist.size())
        , _Dummy(new _Node<value_type>(0, max_level)) {
        for(const auto &elem : _Ilist) {
            insert(elem);
        }
    }
	skiplist(const skiplist<_Ty, _Pr, _Eq> &other) :_Nodecnt(other._Nodecnt)
		, _Dummy(new _Node<value_type>(0, max_level)), _Controller(other._Controller)
		, _Compare(other._Compare), _Equal(other._Equal) {
		_Node<value_type> *curr = other._Dummy->_Forward[0];
		while(curr) {
			insert(curr->_Myval);
    ...
    ~skiplist() {
        _Node<value_type> *curr = _Dummy;
        while(curr) {
            _Node<value_type> *tmp = curr;
            curr = curr->_Forward[0];
            delete tmp;
        }
    }
    ...
    void insert(const_reference _Val) {
        vector<_Node<value_type> *> update(max_level + 1, nullptr);
        _Node<value_type> *curr = _Dummy;
        for(int i = static_cast<int>(_Level); i >= 0; --i) {
            while(curr->_Forward[i]) {
        ...
        _Node<value_type> *newnode = new _Node<value_type>(_Val, level);
    ...
    bool erase(const_reference _Val) {
        ...
        for(int i = static_cast<int>(_Level); i >= 0; --i) {
            while(curr->_Forward[i]) {
                if(_Equal(_Val, curr->_Forward[i]->_Myval)) {
					_Node<value_type> *to_erase = curr->_Forward[i];
                    for(int j = i; j >= 0; --j) {
    ...
    void display() const {
        for(int i = static_cast<int>(_Level); i >= 0; --i) {
            cout << "level " << i << ": ";
            _Node<value_type> *node = _Dummy->_Forward[i];
            while(node != nullptr) {
                cout << node->_Myval << " ";
